...
# SIMPLE PAS ACCOUNT OFFBOARD SCRIPT
# @author cphillipson@pivotal.io
# @version 0.1
#
# Existing user accounts have org and space roles removed for existing org and spaces on a Pivotal Application Service foundation
...
if [ ! -f "$inputfile" ] || [ -z "$orgs" ] || [ -z "$spaces" ]; then
    echo "Usage: offboard-accounts.sh {filename.csv} {comma-separated list of organization names} {comma-separated list of space names}";
    exit 1;
fi
...
do
	...
	echo -e "Processing account offboarding request for: \n\tusername=$user_name\n\torg_role=$org_role\n\tspace_role=$space_role";
	...
	then
		...
		if in_array org_rolearray "$org_role"; then
			for org in "${org_namearray[@]}"; do
				cf unset-org-role "$user_name" "$org" "$org_role";
			done
		else
		...
		if in_array space_rolearray "$space_role"; then
			for org in "${org_namearray[@]}"; do
				for space in "${space_namearray[@]}"; do
					cf unset-space-role "$user_name" "$org" "$space" "$space_role";
				done
			done
			succeeded=true;
		else
	...
	then
		...
		echo "Succeeded!";
		echo "This account has had access removed from organizations [ $orgs ] and spaces [ $spaces ]";
		echo "where the organization role was set to $org_role";
		echo -e "and space role for each space was set to $space_role\n";
	fi
done < "$inputfile";
